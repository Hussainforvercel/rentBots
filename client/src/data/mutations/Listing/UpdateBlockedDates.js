...
const UpdateBlockedDates = {
    ...
    async resolve({ request }, { listId, blockedDates, calendarStatus, isSpecialPrice, startTime, endTime }) {
                ...
                if (blockedDates) {
                    ...
                    if (newBlockedDates && newBlockedDates.length > 0) {
                        const createNewBlockedDates = await ListBlockedDates.bulkCreate(newBlockedDates, {
                            updateOnDuplicate: ['listId', 'blockedDates', 'isSpecialPrice', 'calendarStatus', 'startTime', 'endTime']
                        })
                    }
