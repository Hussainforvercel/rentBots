...
class Page5 extends Component {
  ...
  static propTypes = {
    ...
    change: PropTypes.func.isRequired,  // redux-form change action
  };
  ...
  // NEW: handle selection from PlacesSuggest
  handleLocationSelect = (address) => {
    if (!address || !address.geometry) return;

    const lat = address.geometry.location.lat();
    const lng = address.geometry.location.lng();

    // Update redux-form fields lat & lng
    this.props.change('lat', lat);
    this.props.change('lng', lng);

    // Optionally update location field (formatted address)
    this.props.change('location', address.formatted_address || address.name || '');
  }
    ...
    return (
      <div>
        <PlacesSuggest
          ...
          label={label}
          className={className}
          onSelect={this.handleLocationSelect}  // pass handler here
        />
