...
class BookingButton extends Component {
    ...
    handleClick() {
        const { bookingProcess, listId, guests, startDate,hourlyPrice,isSingleDate, endDate, startTime, endTime, deliveryStatus, roomType } = this.props;
        bookingProcess(
            listId, guests, startDate, endDate, null, startTime,hourlyPrice,isSingleDate, endTime, deliveryStatus, roomType);
    }
    hanldeChange() {
        history.push('/s');
    }
    render() {
        ...
        if (isDisabled || basePrice < 1 || isHost || userBanStatus) {
            disabled = true;
        } else {
            disabled = false;
        }
