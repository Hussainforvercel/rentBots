...
class MapResults extends React.Component {
  ...
  componentDidMount() {
    ...
    if (results && results.length > 0) {
      ...
      results.map((item) => {
        ...
        data["hourlyPrice"] = item.listingData?.hourlyPrice;
  ...
  getMarkerIcon(marker) {
    ...
    try {
      ...
      console.error('Error generating marker icon:', error);
      return '';
    }
  }
  ...
  generateIcon(marker) {
    ...
    const count = (convertedAmount?.toString() || '0').length;
                      ...
                      >
                        <div>
                          <MapListingItem
                            ...
                            hourlyPrice={marker.hourlyPrice}
